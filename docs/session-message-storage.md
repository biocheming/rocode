...
- Repo：`crates/rocode-storage/src/repository.rs`
...
- DB：`sessions.directory TEXT`（`crates/rocode-storage/src/schema.rs`）
...
- `crates/rocode-server/src/routes/session/session_crud.rs`：`list_sessions`
  - 构造 `rocode_session::SessionFilter { directory: query.directory, ... }`
...
也就是说：**directory 过滤是“完全字符串匹配”**，不做二次 canonicalize、不做前缀匹配、不做大小写归一化。

对后续 server 开发的建议（强烈）：

- 如果你希望客户端传“相对路径/未规范化路径”也能过滤：  
  在 `list_sessions` 路由层对 `query.directory` 调用 `resolved_session_directory()` 再过滤。
- 如果你希望支持“列出某目录树下所有会话”：  
...
- SQLite schema 虽然有 `messages.tokens_* / cost / provider_id / model_id` 等列（`crates/rocode-storage/src/schema.rs`），但当前 `MessageRepository::{create, upsert}` 并未写入这些列（只写 `finish/metadata/data`）
...
SQLite schema 里有 `parts` 表（`crates/rocode-storage/src/schema.rs`），设计目标是把 parts 拆出来，以便：
...
- Repository：❌ 当前没有 `count_sessions(...)` 之类的方法
- Server API：❌ 当前没有返回 total 的接口（`list_sessions` 只返回列表）

**分页（LIMIT / cursor）**

- Schema：✅ `ORDER BY updated_at DESC LIMIT ? OFFSET ?` 或 keyset 都可
- Repository：🟡 `SessionRepository::list(project_id, limit)` 支持 `LIMIT`，但 **不支持 OFFSET / cursor**（见 `crates/rocode-storage/src/repository.rs`）
- Server API：❌ `GET /sessions` 的 `limit` 目前会被忽略  
  - 原因：`SessionManager::list_filtered` 没有使用 `filter.limit`，也没有排序（见 `crates/rocode-session/src/session.rs`）
...
> 结论：**会话列表目前并不能“很好支持”分页/offset/总数**。  
> 甚至 `limit` 参数在路由层出现了，但实际不生效，这对后续 server 做标准列表接口会是直接 blocker。
...
- Repository：❌ 没有 `count_messages_for_session(session_id)`
- Server API：❌ `GET /session/{id}/messages` 不返回 total

**分页**

- Schema：✅（建议使用 `(created_at, id)` 做 keyset；也可以 OFFSET）
- Repository：❌ `MessageRepository::list_for_session(session_id)` 永远返回全量（无 limit/offset）
- Server API：🟡 支持 `after + limit`（见 5.6），但它是：
  - 基于内存 vector 的扫描分页
...
> 结论：**消息列表“勉强支持增量拉取”，但对通用 server 的标准分页（offset/keyset + total）支持不完整**。  
> 如果你计划做“单独的消息列表服务/搜索/审计”，建议把分页能力下沉到 storage 层并做可扩展索引。
...
### 7.1 先修正会话列表的“最小可用分页能力”

最小闭环（不改 DB，只改内存 list 的语义）：

- `SessionManager::list_filtered`：
  - 过滤后按 `time.updated DESC` 排序（必要时用 `id` 做 tie-break）
  - 应用 `limit`（让 `GET /sessions?limit=` 真正生效）

这能让前端至少拿到“最近会话”列表，避免 HashMap 遍历顺序导致的乱序。

> 如果你担心行为变更，可以只在 `limit.is_some()` 时排序；但从产品角度，稳定排序通常更正确。
