...
impl CliPanelFrame {
    pub fn boxed(title: &str, footer: Option<&str>, style: &CliStyle) -> Self {
        ...
        let footer_line = footer.map(|text| {
            let content =
                pad_right_display(&truncate_display(&format!(" {} ", text.trim()), chrome_width), chrome_width, '─');
            if style.color {
                format!(
                    "{}{}{}",
                    style.cyan("╰"),
...
pub fn row_char_index_for_display_column(text: &str, row_start: usize, row_end: usize, target_col: usize) -> usize {
