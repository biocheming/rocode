...
fn run_selector(
    ...
    draw_header(&mut stdout, question, header, style)?;

    // Initial draw
    let lines_drawn = draw_options(&mut stdout, options, cursor_pos, &selected, multi, style)?;
    ...
    loop {
        if let Event::Key(key) = event::read()? {
            ...
            draw_options(&mut stdout, options, cursor_pos, &selected, multi, style)?;
        }
    }
}
...
fn draw_options(
    ...
    for (i, opt) in options.iter().enumerate() {
        ...
        let label = if is_cursor {
            style.bold(&opt.label)
        } else {
            opt.label.clone()
        };

        let desc = match &opt.description {
            Some(d) if is_cursor => format!(" {}", style.dim(d)),
            _ => String::new(),
        };
    ...
    let is_cursor = cursor_pos == other_idx;
    let pointer = if is_cursor {
        style.bold_cyan("❯")
    } else {
        " ".to_string()
...
fn cleanup_terminal(out: &mut impl Write, lines_drawn: usize) -> io::Result<()> {
    ...
    // Also erase the header lines (blank + question + blank = 3, or +1 for header label = 4)
    // We erase a few extra lines to clean up
    for _ in 0..4 {
        execute!(
            out,
            cursor::MoveUp(1),
