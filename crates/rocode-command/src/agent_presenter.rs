use crate::output_blocks::{
    ...
    MessagePhase, MessageRole, OutputBlock, QueueItemBlock, SchedulerStageBlock,
    SessionEventBlock, SessionEventField, StatusBlock, ToolBlock, ToolPhase,
...
pub fn history_session_event_to_web(
    ...
    output_block_to_web(&OutputBlock::SessionEvent(SessionEventBlock {
        ...
        fields: fields
            .into_iter()
            .map(|(label, value, tone)| SessionEventField {
                label: label.into(),
                value: value.into(),
                tone,
            })
...
pub fn output_block_to_web(block: &OutputBlock) -> serde_json::Value {
    match block {
        ...
        OutputBlock::SchedulerStage(SchedulerStageBlock {
            ...
            "decision": decision.as_ref().map(|decision| json!({
                "kind": decision.kind,
...
mod tests {
    ...
    fn queue_item_block_to_web_shape() {
        let web = output_block_to_web(&OutputBlock::QueueItem(crate::output_blocks::QueueItemBlock {
            position: 4,
            text: "finish docs sync".to_string(),
        }));
        assert_eq!(web.get("kind").and_then(|value| value.as_str()), Some("queue_item"));
        assert_eq!(web.get("position").and_then(|value| value.as_u64()), Some(4));
        assert_eq!(
            web.get("display")
    ...
    fn scheduler_stage_web_shape_includes_decision_block() {
        let web = output_block_to_web(&OutputBlock::SchedulerStage(SchedulerStageBlock {
