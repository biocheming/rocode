...
impl AgentExecutor {
    ...
    async fn execute_subsession_with_cancel_token(
        ...
        let model_ref = self.agent.model.as_ref().map(|m| OrchestratorModelRef {
    ...
    pub async fn execute_reported(
        ...
        self.execute_streaming_impl(user_message).await
    }
    ...
    pub async fn execute_rendered(
        ...
        let report = self.execute_streaming_impl(user_message).await?;
        Ok(report.outcome)
    }
