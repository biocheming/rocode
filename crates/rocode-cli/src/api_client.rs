...
pub use rocode_tui::api::{
    AgentInfo, CompactResponse, CreateSessionRequest, ExecuteRecoveryRequest,
    ExecuteShellRequest, ExecutionModeInfo, FullProviderListResponse, KnownProvidersResponse,
    McpAuthStartInfo, McpStatusInfo, MessageInfo, MessageTokensInfo,
    PromptRequest, ProviderListResponse, QuestionInfo, RecoveryActionKind,
    RevertRequest, RevertResponse, SessionExecutionTopology, SessionInfo,
    SessionRecoveryProtocol, SessionStatusInfo, ShareResponse, UpdateSessionRequest,
};
...
impl CliApiClient {
    ...
    pub async fn disconnect_mcp(&self, name: &str) -> anyhow::Result<bool> {
        ...
        let resp = self.client.post(&url).send().await?;
        let bytes =
            Self::expect_success(resp, &format!("disconnect MCP `{}`", name)).await?;
        Ok(serde_json::from_slice::<bool>(&bytes).unwrap_or(true))
    }
