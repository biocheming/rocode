...
use super::types::{
    ActiveReasoning, CodeInterpreterState, FinishReason,
    LogprobEntry, OngoingToolCall, OutputItemAddedItem, OutputItemDoneItem,
    ResponsesIncludeValue, ResponsesStreamChunk, ResponsesUsage,
    map_openai_response_finish_reason,
};
...
pub(super) fn parse_output_items(output: &[Value]) -> (Vec<ContentPart>, bool, Vec<Vec<LogprobEntry>>) {
...
pub(super) fn insert_opt_string(obj: &mut serde_json::Map<String, Value>, key: &str, value: Option<String>) {
    if let Some(value) = value {
        obj.insert(key.to_string(), Value::String(value));
    }
}

pub(super) fn insert_opt_u64(obj: &mut serde_json::Map<String, Value>, key: &str, value: Option<u64>) {
    if let Some(value) = value {
        obj.insert(key.to_string(), Value::Number(value.into()));
    }
}

pub(super) fn insert_opt_bool(obj: &mut serde_json::Map<String, Value>, key: &str, value: Option<bool>) {
    if let Some(value) = value {
        obj.insert(key.to_string(), Value::Bool(value));
    }
}

pub(super) fn insert_opt_value(obj: &mut serde_json::Map<String, Value>, key: &str, value: Option<Value>) {
    if let Some(value) = value {
