...
use crate::session_runtime::{assistant_visible_text, scheduler_stage_block_from_message};
...
use rocode_command::output_blocks::{
    MessageBlock, MessageRole, OutputBlock, ToolBlock,
};
...
impl StreamSnapshotEmitter {
    ...
    async fn emit_scheduler_stage(
        ...
        let signature =
            output_block_to_web(&OutputBlock::SchedulerStage(block.clone())).to_string();
        ...
        emit_output_block(
            tx,
            OutputBlock::SchedulerStage(block),
            Some(message.id.as_str()),
        )
        .await;
        true
...
mod tests {
    ...
    fn scheduler_stage_message_falls_back_to_metadata_title_when_heading_missing() {
        ...
        assert_eq!(block.text, "Decision pending on the unresolved task ledger.");
    }
}
