...
Remind the user: plan first, then run `/start-work` for execution.
...
- plan-8: Delete draft file and guide user to `/start-work {name}`
...
- `/start-work` begins the execution workflow with the orchestrator
...
- **Handoff**: once ready, guide the user to `/start-work` and delete the draft
...
- Execution Status
In Execution Status, state clearly that code execution has not been performed in this workflow and guide the user to `/start-work`.
</active_stage>
"###;
