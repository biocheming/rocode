...
{}",
    ...
    push_optional_section(&mut sections, "Metis Review", input.metis_review);
    push_optional_section(&mut sections, "Momus Feedback", input.momus_feedback);
    sections.push(format!(
        "## Current Plan
{}",
        input.current_plan
...
{}",
    ...
    push_optional_section(&mut sections, "Momus Review", input.momus_review);
...
- Code execution has not been performed here"
    ...
    sections.push(
        "## Handoff Charter
End this workflow with a reviewed planning handoff. If the plan is approved, make `/start-work` the explicit next action. If Momus still blocks the plan or decisions remain unresolved, say so clearly and do not imply execution is ready."
            .to_string(),
    );
    sections.join(
        "
