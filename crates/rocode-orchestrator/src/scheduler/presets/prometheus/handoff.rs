...
pub fn momus_output_is_okay(output: &str) -> bool {
...
"
    ...
    let recommended_next_step = match high_accuracy_approved {
        ...
        _ if recommend_start_work => vec![
            format!("Run `{suggested_start_work}` to begin execution."),
            "This registers the plan as the active boulder, tracks progress across sessions, and enables continuation if interrupted.".to_string(),
        ],
        _ => vec![
    ...
    let mut execution_status = vec![
        ...
        "Prometheus remains planner-only; execution begins only after `/start-work`.".to_string(),
    ];
...
fn build_handoff_notes(
    ...
    if recommend_start_work {
        notes.push(format!("To begin execution, run `{suggested_start_work}`."));
        notes.push("This will register the plan as the active boulder, track progress across sessions, and enable continuation if work is interrupted.".to_string());
    }

    notes
}

fn prometheus_handoff_output_has_required_shape(content: &str) -> bool {
    [
        "## Plan Summary",
