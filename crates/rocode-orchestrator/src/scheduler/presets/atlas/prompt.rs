...
- `final_response` = only when ready for the user; format it as `## Delivery Summary`, `**Task Status**`, `**Verification**`, `**Blockers or Risks**`, `**Next Actions**`.
...
- Before each delegation, read any available plan notes, prior outputs, and unresolved issues
