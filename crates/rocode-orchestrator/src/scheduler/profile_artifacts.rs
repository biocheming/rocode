...
impl SchedulerProfileOrchestrator {
    pub(super) fn artifact_path_mut<'a>(
    ...
    pub(super) fn artifact_path_ref<'a>(
        kind: SchedulerArtifactKind,
        state: &'a SchedulerProfileState,
    ) -> &'a Option<String> {
        match kind {
    ...
    pub(super) fn render_preset_draft_artifact(
        original_input: &str,
        plan: &SchedulerProfilePlan,
        ...
        plan.compose_draft_artifact(SchedulerDraftArtifactInput {
            ...
            handoff_choice: state.preset_runtime.handoff_choice.as_deref(),
            planning_artifact_path: state.preset_runtime.planning_artifact_path.as_deref(),
            draft_artifact_path: state.preset_runtime.draft_artifact_path.as_deref(),
        })
    }

    pub(super) fn sync_preset_draft_artifact(
        original_input: &str,
        plan: &SchedulerProfilePlan,
        state: &mut SchedulerProfileState,
        ctx: &OrchestratorContext,
    ) -> Result<(), OrchestratorError> {
        let Some(draft) = Self::render_preset_draft_artifact(original_input, plan, state) else {
            return Ok(());
        };
