...
use super::{
    SchedulerExecutionGateDecision, SchedulerExecutionGateStatus, SchedulerExecutionVerificationMode,
    SchedulerInternalStageSpec, SchedulerProfileOrchestrator, SchedulerProfilePlan,
...
impl<'a> SchedulerExecutionService<'a> {
    ...
    async fn execute_execution_round(
        ...
        SchedulerExecutionCapabilityAdapter::new(self.orchestrator, self.plan, self.ctx)
            .execute_execution_path(execution_input, child_mode, allow_execution_fallback, None)
            .await
    }
    ...
    async fn execute_coordination_verification(
        ...
        } else if matches!(verification_mode, SchedulerExecutionVerificationMode::Required)
            && matches!(
    ...
    fn resolve_autonomous_verification_prompt(
        &self,
        stage: SchedulerInternalStageSpec,
    ) -> String {
        let profile_suffix = super::profile_prompt_suffix(self.plan);
        self.plan
