...
use crate::runtime::events::{
    FinishReason, LoopError, LoopEvent, NeverCancel, StepBoundary, ToolCallReady, ToolResult,
};
...
impl Orchestrator for SkillListOrchestrator {
    async fn execute(
        ...
        let outcome = run_loop(&model, &tools, &mut sink, &policy, &cancel, messages)
            .await
            .map_err(|e| match e {
...
impl LoopSink for SkillListSink {
    async fn on_event(&mut self, ev: &LoopEvent) -> Result<(), LoopError> {
        match ev {
            ...
            LoopEvent::ToolCallReady(tc) => self.step_tool_calls.push(tc.clone()),
            _ => {}
        }
        Ok(())
    }
...
mod tests {
    use super::*;
    use crate::{
        AgentDescriptor, AgentResolver, ExecutionContext, LifecycleHook, ModelRef, ModelResolver,
        OrchestratorContext, ToolExecError, ToolExecutor, ToolOutput,
    };
    use async_trait::async_trait;
    use futures::stream;
    use serde_json::json;
