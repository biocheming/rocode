...
pub use tools_and_output::{
    ...
    prioritize_tool_definitions, resolve_tools, resolve_tools_with_mcp,
    resolve_tools_with_mcp_registry, structured_output_system_prompt, was_plan_agent, ResolvedTool,
...
impl SessionPrompt {
    ...
    pub async fn prompt_with_update_hook(
        ...
        event_broadcast: Option<EventBroadcastHook>,
        agent_lookup: Option<Arc<dyn Fn(&str) -> Option<rocode_tool::TaskAgentInfo> + Send + Sync>>,
        ask_question_hook: Option<AskQuestionHook>,
    ) -> anyhow::Result<()> {
        ...
        let input_variant = input.variant.clone();
    ...
    async fn run_runtime_step(
        ...
        let model = SimpleModelCaller {
            ...
            config: SimpleModelCallerConfig {
                model_id: model_id.to_string(),
                max_tokens: agent_params.max_tokens.unwrap_or(8192),
        ...
        let tools = SessionStepToolDispatcher {
            ...
            agent_name: agent_name.unwrap_or_default().to_string(),
            abort_token: token,
            tool_registry,
            provider,
            provider_id: provider_id.to_string(),
        ...
        let mut sink = SessionStepSink::new(
            session,
            assistant_index,
            update_hook.as_ref().map(|&x| x),
            event_broadcast.as_ref().map(|&x| x),
            step_complete,
        );
        ...
        let outcome = run_loop(&model, &tools, &mut sink, &policy, &cancel, chat_messages).await;
        ...
        match outcome {
            ...
            Err(RuntimeLoopError::ToolDispatchError { tool, error }) => Err(anyhow::anyhow!(
    ...
    async fn process_pending_subtasks(
        ...
        for subtask in &pending {
            ...
            match Self::execute_persisted_subsession_prompt(
                ...
                None, // No cancellation token for compaction-driven subsession prompts.
