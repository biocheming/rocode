...
fn render_scheduler_stage_part(
    ...
    lines.extend(render_stage_runtime_lines(metadata, theme));
...
fn render_stage_runtime_lines(
    ...
    let status = metadata
        ...
        .and_then(Value::as_str)
        .unwrap_or("running");
    let step = metadata.get("scheduler_stage_step").and_then(Value::as_u64);
    let step_limit = metadata
        .get("scheduler_stage_loop_budget")
        .and_then(Value::as_str)
        .and_then(parse_step_limit);
    ...
    let available_category_count = metadata
        ...
        .and_then(Value::as_u64);
    let active_skills = metadata
        .get("scheduler_stage_active_skills")
        .and_then(Value::as_array)
        .filter(|arr| !arr.is_empty());
    let active_agents = metadata
        .get("scheduler_stage_active_agents")
        .and_then(Value::as_array)
        .filter(|arr| !arr.is_empty());
    let active_categories = metadata
        ...
        if let Some(skills) = active_skills {
            ...
            if !values.is_empty() {
                lines.push(Line::from(vec![
                    ...
                    Span::styled(values.join(", "), cap_value_style),
                ]));
            }
        }
...
mod tests {
    ...
    fn message_with_stage_runtime_meta(
        ...
        if let Some(loop_budget) = loop_budget {
            metadata.insert(
    ...
    fn message_with_gate_meta(
        ...
        let mut message = message_with_stage_runtime_meta(
            ...
            Some(1),
            Some("running"),
            Some("Decide whether the coordination loop can finish."),
