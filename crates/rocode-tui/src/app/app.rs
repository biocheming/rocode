...
use crate::api::{
    ...
    MessageInfo, QuestionInfo, RecoveryProtocolStatus as ApiRecoveryProtocolStatus,
    RecoveryActionKind as ApiRecoveryActionKind, SessionExecutionNode, SessionInfo,
...
use crate::components::{
    ...
    ToolCallItem, RecoveryActionDialog, RecoveryActionItem, OTHER_OPTION_ID, OTHER_OPTION_LABEL,
};
...
impl App {
    ...
    fn execute_typed_interactive_command(
        ...
        match command {
            ...
            InteractiveCommand::ToggleSidebar | InteractiveCommand::ToggleActive
            | InteractiveCommand::ScrollUp | InteractiveCommand::ScrollDown
    ...
    fn handle_show_recovery_actions(&mut self) {
    ...
    fn handle_execute_recovery_action(&mut self, selector: &str) {
        let Some(session_id) = self.current_session_id() else {
            self.alert_dialog
        ...
        let Some(action) = resolve_recovery_action_selection(&recovery, selector) else {
            self.alert_dialog
                .set_message("Unknown recovery action. Open /recover and select one from the list.");
            self.alert_dialog.open();
            return;
        };
    ...
    fn execution_status_blocks(&self, session_id: &str) -> Vec<StatusBlock> {
        let Some(client) = self.context.get_api_client() else {
            return vec![
    ...
    fn recovery_status_blocks(&self, session_id: &str) -> Vec<StatusBlock> {
        let Some(client) = self.context.get_api_client() else {
            return vec![
    ...
    fn check_scheduler_handoff(&mut self, session_id: &str) {
        ...
        self.dispatch_prompt_to_session(
            session_id,
            handoff_command.unwrap_or_else(|| "/start-work".to_string()),
            None,
            Some(target_mode),
...
fn append_execution_status_node(
...
fn recovery_status_blocks_from_protocol(recovery: &SessionRecoveryProtocol) -> Vec<StatusBlock> {
    ...
    if let Some(summary) = recovery.summary.as_deref() {
        blocks.push(match recovery.status {
            ApiRecoveryProtocolStatus::Recoverable => StatusBlock::success(format!("- {}", summary)),
...
fn recovery_action_key(
    action: &crate::api::RecoveryActionInfo,
    index: usize,
) -> String {
...
fn recovery_action_items(recovery: &SessionRecoveryProtocol) -> Vec<RecoveryActionItem> {
    recovery
        .actions
        .iter()
        .enumerate()
...
fn resolve_recovery_action_selection<'a>(
    ...
    recovery.actions.iter().enumerate().find_map(|(index, action)| {
